...
def main():
    ...
    for chapter_idx, chapter in enumerate(data):
        if isinstance(chapter, dict) and 'articles' in chapter:
            chapter_title = chapter.get('title', f'章{chapter_idx+1}')
            articles = chapter['articles']
            print(f"\n{chapter_title}: {len(articles)}個の条文")
            ...
            for article_idx, article in enumerate(articles):
                if isinstance(article, dict):
                    ...
                    text = article.get('text', article.get('body', article.get('content', '')))
                    
                    # 64条関連をチェック
                    if '64' in article_number or '六十四' in article_number or '64' in title or '六十四' in title:
                        print(f"  ★ 第64条発見! 位置: 章{chapter_idx+1}-条文{article_idx+1}")
                        print(f"     条文番号: {article_number}")
                        print(f"     タイトル: {title}")
                        print(f"     テキスト: {text[:200]}...")
                        print(f"     全フィールド: {list(article.keys())}")
                        
                        # この条文を使ってテストしてみる
                        test_highlight_with_article(article)
                        return
    ...
    print(f"\n総条文数: {len(all_articles)}")
    
    # 64条が見つからない場合、類似の条文で代用
    print("\n=== 64条が見つからないため、代替条文でテスト ===")
    for article in all_articles:
        if isinstance(article, dict):
            article_number = article.get('article_number', '')
            if '60' in article_number or '65' in article_number:  # 近い番号で試す
                print(f"代替条文でテスト: {article_number}")
                test_highlight_with_article(article)
                return

def test_highlight_with_article(article):
    """指定された条文でハイライトテストを実行"""
    print(f"\n=== ハイライトテスト開始 ===")
    print(f"条文番号: {article.get('article_number', '不明')}")
    ...
    for category, entity_list in entities.items():
        ...
        if len(entity_list) > 0:
            print(f"    例: {entity_list[0]}")
    
    # ハイライト処理をテスト
    if entities:
        highlighted = test_highlight_entities(text, entities, selected_category="すべて")
...
<body>
    <h1>ハイライト結果 - {article.get('article_number', '不明')}</h1>
...
""")
        ...
        print("エンティティが見つかりません")

def test_highlight_entities(text, entities, selected_category=None):
